...
namespace Broccoli.Manager
{
	...
	public class AssetManager {
		...
		*/
		...
		bool SavePrefab (bool generateBillboard, bool isTreeCreator) {
			...
			if (prefabIsValid) {
				...
				if (assetMeshes.Count == 1 && !generateBillboard) {
					...
					Mesh mainMesh = CreateMeshGameObject (prefabGameObject, 0, pass);
					...
					for (int pass = assetMeshes.Count; pass >= 1; pass--) {
						...
						Mesh lodMesh = CreateMeshGameObject (lodGameObject, i, pass);
						lodMesh.name = "LOD_" + i;
		...
		Mesh CreateMeshGameObject (GameObject gameObject, int lodLevel, int pass = 1) {
			Mesh mainMesh = null;
			#if UNITY_EDITOR
			...
			if (TreeFactory.GetActiveInstance ().treeFactoryPreferences.prefabIncludeAssetsInsidePrefab) {
				...
				string folderPath = GetPrefabTextureFolder ();
				string meshPath = folderPath + "/LOD_" + lodLevel + ".asset";
